...
Hyprlang::CParseResult onNewButton(const char* K, const char* V) {
    ...
    btn.size = size;
    btn.icon = vars[2];
    g_pGlobalState->buttons.push_back(std::move(btn));
...
int newLuaButton(lua_State* L) {
...

    {
        ...
        if (lua_isnumber(L, -1))
            button.height = lua_tonumber(L, -1);
    }

    {
        ...
        if (lua_isnumber(L, -1))
            button.round = lua_tointeger(L, -1);
    }

    {
        ...
        if (lua_isnumber(L, -1))
            button.icon_size = lua_tointeger(L, -1);
    }
...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle) {
    ...
    // Force fontconfig & pango in Hyprland process to recognize lucide font
    FcInitReinitialize();
    const char* home = getenv("HOME");
    if (home) {
        std::string p1 = std::string(home) + "/.local/share/fonts/lucide/lucide.ttf";
        FcConfigAppFontAddFile(nullptr, (const FcChar8*)p1.c_str());
    }
    ...
    g_pGlobalState->config.barTitlePadding     = makeShared<Config::Values::CIntValue>("plugin:hyprbars:bar_title_padding", "Padding of the title text", -1);
    ...
    HyprlandAPI::addConfigValueV2(PHANDLE, g_pGlobalState->config.barTitlePadding);
