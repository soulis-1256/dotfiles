...
PanelWindow {
    ...
    readonly property real effectiveBarThickness: (FrameTransitionState.effectiveFrameEnabled && usesFrameBarChrome) ? SettingsData.frameBarSize : ((barConfig && barConfig.fullHeightWidgets) ? Theme.barHeight : Theme.snap(Math.max(barWindow.widgetThickness + (barConfig?.innerPadding ?? 4) + 4, Theme.barHeight - 4 - (8 - (barConfig?.innerPadding ?? 4))), _dpr))
    ...
    readonly property real widgetThickness: (barConfig && barConfig.fullHeightWidgets) ? Theme.barHeight : Theme.snap(Math.max(20, 26 + (barConfig?.innerPadding ?? 4) * 0.6), _dpr)
