...
PanelWindow {
    ...
    readonly property real configuredBarHeight: {
        const h = barConfig?.barHeight;
        if (typeof h === "number" && h > 0)
            return h;
        return Theme.barHeight;
    }
    readonly property real effectiveBarThickness: (FrameTransitionState.effectiveFrameEnabled && usesFrameBarChrome) ? SettingsData.frameBarSize : ((barConfig && barConfig.fullHeightWidgets) ? barWindow.configuredBarHeight : Theme.snap(Math.max(barWindow.widgetThickness + (barConfig?.innerPadding ?? 4) + 4, barWindow.configuredBarHeight - 4 - (8 - (barConfig?.innerPadding ?? 4))), _dpr))
    ...
    readonly property real widgetThickness: (barConfig && barConfig.fullHeightWidgets) ? barWindow.configuredBarHeight : Theme.snap(Math.max(20, 26 + (barConfig?.innerPadding ?? 4) * 0.6), _dpr)
